    ...
    initialiseFullCalendarJs() {
        ...
        $(ele).fullCalendar({
            ...
            select: function (startDate, endDate) {                
                ...
                //let stDate = moment(startDate).format('MMM DD, YYYY h:mm A');
                //let edDate = moment(endDate).format('YYYY-MM-DDTHH:mm:ss');
                
                let stDate = moment(startDate).utc().toISOString();
                let edDate = moment(endDate).utc().toISOString();
                
                //let stDate = moment(startDate).utcOffset("+6:30").toISOString();
                //let edDate = moment(endDate).utcOffset("+6:30").toISOString();

                console.log(edDate);
                //let edDate = '2019-03-06T10:17:36.789Z';
   ...
   //To save the event
    handleSave(event) {
        ...
        this.template.querySelectorAll('lightning-input').forEach(ele => {
            if(ele.name === 'title'){
               //this.title = ele.value.isnull() ? 'All day visit' : ele.value;
           ...
           if(ele.name === 'start'){
            ...
            // if(ele.name === 'allday'){
            //     this.allDayEvent = ele.value;
            // }            
        });
       
        //format as per fullcalendar event object to create and render
        //let newevent = {title : this.title, start : this.startDate, end: this.endDate, allDay: true};
